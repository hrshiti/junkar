...
async function check() {
    ...
    try {
        ...
        // Search for the scrapper with a more flexible query
        const scrapper = await db.collection('scrappers').findOne({ 
            phone: { $regex: "9521073590" } 
        });
        ...
        if (scrapper) {
            ...
            console.log('Scrapper with phone 9521073590 not found in scrappers collection');
            
            // Check if it's in the users collection with a scrapper role
            ...
            if (user) {
                ...
                console.log('User with phone 9521073590 not found either');
            }
        }
