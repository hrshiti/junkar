        ...
        for (const dbInfo of dbsList.databases) {
            ...
            for (const col of collections) {
                if (col.name === 'users' || col.name === 'scrappers') {
                    ...
                    log(`  Collection ${col.name}: Found ${count} matches`);
                    if (count > 0) {
                        ...
                        log(`  Matches in ${dbName}.${col.name}: ${JSON.stringify(docs.map(d => ({ name: d.name, phone: d.phone, kycStatus: d.kyc?.status, db: dbName })), null, 2)}`);
                    }
                }
            }
        }
